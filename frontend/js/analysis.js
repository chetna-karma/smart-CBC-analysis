...
// Compiled CBC Analyzer page logic
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const validateField = (inputEl, val, min, max, name) => {
    ...
    const bindSliderInput = (inputId, rangeId, min, max, name) => {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if (hasErrors) {
            window.showToast('Please correct form errors before submitting.', 'error');
            return;
        }
        ...
        const result = await window.API.post('/reports', {
            hemoglobin,
            wbc,
            platelets,
            rbc,
        ...
        if (result.success && result.data) {
            window.showToast('CBC Report analyzed successfully!', 'success');
            ...
            window.showToast(result.message || 'Analysis failed. Please check inputs.', 'error');
            if (result.errors) {
