...
// Compiled Profile Settings Page logic
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const loadProfile = async () => {
        const result = await window.API.get('/auth/profile');
        if (result.success && result.data) {
            ...
            const reportsResult = await window.API.get('/reports');
            let reports = [];
    ...
    const clearInputError = (input, errorId) => {
    ...
    const showInputError = (input, errorId, message) => {
    ...
    profileForm.addEventListener('submit', async (e) => {
        ...
        const result = await window.API.request('/auth/profile', {
            method: 'PUT',
