...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const loadDashboard = async () => {
        try {
            ...
            if (result.success && result.data) {
                ...
                if (lowRiskEl) lowRiskEl.textContent = stats.risk_indicators.low.toString();
                if (medRiskEl) medRiskEl.textContent = stats.risk_indicators.medium.toString();
                if (highRiskEl) highRiskEl.textContent = stats.risk_indicators.high.toString();
                ...
                if (recentTableBody) {
                    if (stats.recent_reports && stats.recent_reports.length > 0) {
                        ...
                        recentTableBody.innerHTML = stats.recent_reports.map((r: any) => {
                        ...
                        recentTableBody.querySelectorAll('.btn-view-report').forEach(btn => {
                            btn.addEventListener('click', (e) => {
                                const reportId = (e.target as HTMLElement).getAttribute('data-id');
                                if (reportId) {
            ...
            showToast('Error displaying metrics dashboard.', 'error');
        }
    };

    // Load parameter data and render initial trend charts
    const loadTrendData = async () => {
        const result = await API.get('/reports?sort=oldest');
        if (result.success && result.data) {
    ...
    const handleCsvExport = () => {
        if (allReportsForChart.length === 0) {
            showToast('No blood reports to export.', 'warning');
            return;
        }
    ...
    const generateNotifications = (report: any) => {
        const notifContainer = document.getElementById('notifications-list');
        if (!notifContainer) return;
        
        // Clear except system default greeting
        ...
        // Parse parameters to trigger notification alerts
        const addAlert = (title: string, desc: string, icon = '⚠️') => {
    ...
    const paramSelect = document.getElementById('trend-parameter-select') as HTMLSelectElement | null;
    if (paramSelect) {
        paramSelect.addEventListener('change', (e) => {
            activeParameter = (e.target as HTMLSelectElement).value;
            updateTrendChart();
        });
    }
...
// Update the interactive trend line chart
function updateTrendChart() {
    ...
    // Map categories and series values
    ...
    // Determine bounds and configurations for the parameters
    let label = 'Value';
    ...
    if (chartEl) {
        chartEl.innerHTML = '';
        chartInstance = new (window as any).ApexCharts(chartEl, options);
